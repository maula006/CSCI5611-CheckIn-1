...
float speed = 100;
float maxAccel = 1; //The maximum change in speed allowed.
float damping = 0.1; //how much velocity is removed if the ball passes through the point.

void update(float dt){
  Vec2 mousePos = new Vec2(mouseX, mouseY);
  Vec2 dir = mousePos.minus(pos); //Should be vector pointing from pos to MousePos
  if (dir.length() > 0) dir.normalize();
  if (mousePos.distanceTo(pos) < vel.length()){ // when the speed will overshoot the point, set ball to point. (kept the same)
    vel = new Vec2(0,0);
    pos = mousePos;
  } else {
    Vec2 newVel = dir.times(speed*dt); //Scale the speed according to real delta time.
    if (newVel.length() - vel.length() > maxAccel){ //If the acceleration is too much, increase the speed by maxAccel.
      vel.add(dir.times(maxAccel)); //accelerate with max acceleration
    } else {
      vel = newVel;
    }
  }
...
void draw(){
  ...
  circle(pos.x, pos.y,r*2); //Question: Why is it radius*2 ? 
                            //THe third parameter is the "extent", that is, the "width and height" of the circle.
                            //This is probably because the function is adapted from the ellipse() which takes that argument.
                            //As such, the width and height of the circle is the same as its diameter, radius*2.
}
