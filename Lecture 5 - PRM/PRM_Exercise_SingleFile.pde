...
TODO:
  ...
  2. Currently edge connections of any length are allowed. Update the connectNeighbors()
     ...
     - This is because the BFS algorithm looks for the lowest amount of edges between the start and finish, without considering the length of the edge itself.
       Therefore, to prevent this bias, the nodes will be connected without too much distance between them.
...
boolean pointInBox(Vec2 boxTopLeft, float boxW, float boxH, Vec2 pointPos){// 100,100 w100 h250
  //TODO: Return true if the point is actually inside the box
  Vec2 boxBottomRight = new Vec2(boxTopLeft.x + boxW, boxTopLeft.y + boxH);
  println("botrightvec: ", boxBottomRight.x, boxBottomRight.y);
  if (pointPos.x >= boxTopLeft.x && pointPos.x <= boxBottomRight.x && pointPos.y >= boxTopLeft.y && pointPos.y <= boxBottomRight.y){
    return true; 
  }
  return false;
}
...
void generateRandomNodes(Vec2[] circleCenters, float[] circleRadii, Vec2 boxTopLeft, float boxW, float boxH){
  for (int i = 0; i < numNodes; i++){
    ...
    boolean insideSquare = pointInBox(boxTopLeft, boxW, boxH, randPos);
    while (insideAnyCircle || insideSquare){
      randPos = new Vec2(random(width),random(height));
      insideAnyCircle = pointInCircleList(circleCenters,circleRadii,randPos);
      insideSquare = pointInBox(boxTopLeft, boxW, boxH, randPos);
    }
    nodePos[i] = randPos;
  }
}
...
void connectNeighbors(){
  for (int i = 0; i < numNodes; i++){
    ...
    for (int j = 0; j < numNodes; j++){
      ...
      if (!circleListCheck.hit && !boxCheck.hit && distBetween <= 200){ //add conditional to only allow if distance is 200px or less.
        neighbors[i].add(j);
      }
    }
  }
