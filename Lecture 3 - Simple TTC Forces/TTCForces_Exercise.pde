...
float k_goal = 1.2;  //TODO: Tune this parameter to agent stop naturally on their goals
float k_avoid = 70;
float agentRad = 35;
float goalSpeed = 100;
...
float computeTTC(Vec2 pos1, Vec2 vel1, float radius1, Vec2 pos2, Vec2 vel2, float radius2){
  //Vec2 v = vel2.minus(vel1);//v = v_b - v_a (velocity difference)
  //Vec2 w = pos2.minus(pos1);//w = x_2 - x_1 (position difference)
  //float a = dot(v, v); 
  //float b = 2 * dot(w, v);
  //float c = dot(w, w) - ((radius1+radius2) * (radius1+radius2));
  //float tau1 = (-b + sqrt((b*b)-(4*a*c))) / (2 * a);
  //float tau2 = (-b - sqrt((b*b)-(4*a*c))) / (2 * a);
  //if (tau1 < 0 && tau2 < 0) return -1f; //two negatives
  //if ((tau1 < 0 && tau2 > 0) || (tau2 < 0 && tau1 > 0)) return 0f;//one positive
  //if (tau1 > tau2) return tau2;//two positive
  //return tau1;
  //didn't see the other function, whoops
...
Vec2 computeAgentForces(int id){
  ...
  Vec2 goalVel = (goalPos[id].minus(agentPos[id])).normalized().times(goalSpeed); //1. 
  Vec2 goal_force = (goalVel.minus(agentVel[id])).times(k_goal); //smooth movement by using an acceleration. (also, it should be "goal accel" right?)
  acc.add(goal_force);
  
  if (agentPos[id].distanceTo(goalPos[id]) > agentRad){ //if any part of the agent is in the goal, don't calculate avoidance.
    //add avoidance force
    for (int j = 0; j < maxNumAgents; j++){
      ...
      if (ttc < 0) continue;
      Vec2 id_future = agentPos[id].plus(agentVel[id].times(ttc)); //determine future position of collision
      Vec2 j_future = agentPos[j].plus(agentVel[j].times(ttc)); 
      Vec2 relFutureDirection = (id_future.minus(j_future)).normalized(); //get the direction from id to j upon collision
      acc.add(relFutureDirection.times(k_avoid * (1/ttc)));
    }
  }
  return acc;
}
...
void moveAgent(float dt){
  ...
  for (int i = 0; i < numAgents; i++){
    if (agentPos[i].distanceTo(goalPos[i]) < 0.01){
      agentAcc[i] = agentVel[i].mul(-1);
    } else{
